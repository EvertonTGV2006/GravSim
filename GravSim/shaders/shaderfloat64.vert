...
void main() {
    ...
    float mass = float(particleIn.velocity.w);
        gl_Position = ubo.proj*ubo.view*constants.model*vec4(inPosition*mass+vec3(particleIn.position.xyz), 1.0);
        fragPos = (constants.model*vec4(inPosition+vec3(particleIn.position.xyz), 1.0)).xyz;
        double velMod = length(particleIn.velocity.xyz);
        velMod = clamp(velMod, velMin, velMax);
        double velModAdj = (velMod - velMin) * 360 / (velMax - velMin); 

        fragColor = HSVtoRGB(float(velModAdj), 1.0f, 1.0f);
